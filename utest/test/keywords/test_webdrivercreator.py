...
def test_parse_capabilities(creator):
    ...
    for browser in [None, 'safari', 'headlesschrome', 'foobar']:
        caps = creator._parse_capabilities({'key1': 'value1', 'key2': 'value2'}, browser)
        expected = {'desired_capabilities': {'key1': 'value1', 'key2': 'value2'}}
        assert caps == expected
...
def test_capabilities_resolver_chrome(creator):
    ...
    resolved_caps = creator._remote_capabilities_resolver({'capabilities': {'version': '73.0.3683.86'}},
...
def test_htmlunit_with_js_no_browser_name(creator):
    ...
    when(webdriver).Remote(command_executor='None',
                           ...
                           browser_profile=None, options=None,
                           file_detector=file_detector).thenReturn(expected_webdriver)
    driver = creator.create_htmlunit_with_js({'desired_capabilities': {'key': 'value'}}, None)
    assert driver == expected_webdriver
