...
def test_log_file_with_index_exist(creator):
    log_file = os.path.join(creator.output_dir, 'firefox-{index}.log')
    with open(os.path.join(creator.output_dir, log_file.format(index='1')), 'w') as file:
        file.close()
    file_name = creator.creator._get_log_path(log_file)
...
def test_create_chrome_with_service_log_path_real_path(creator):
    log_file = os.path.join(creator.output_dir, 'firefox-{index}.log')
    expected_webdriver = mock()
    when(webdriver).Chrome(options=None, service_log_path=log_file,
                           executable_path='chromedriver').thenReturn(expected_webdriver)
...
def test_create_headlesschrome_with_service_log_path_real_path(creator):
    ...
    when(webdriver).Chrome(options=options, service_log_path=log_file,
                           executable_path='chromedriver').thenReturn(expected_webdriver)
...
def test_create_firefox_with_service_log_path_real_path(creator):
    ...
    when(webdriver).Firefox(options=None, firefox_profile=profile, executable_path='geckodriver',
                            service_log_path=log_file).thenReturn(expected_webdriver)
    driver = creator.creator.create_firefox({}, None, ff_profile_dir=None, service_log_path=log_file)
    assert driver == expected_webdriver
...
def test_create_headlessfirefox_with_service_log_path_real_path(creator):
    ...
    driver = creator.creator.create_headless_firefox({}, None, ff_profile_dir=None, service_log_path=log_file)
    assert driver == expected_webdriver
